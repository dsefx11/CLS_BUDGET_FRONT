...
export class BR_InvestComponent implements OnInit {
    ...
    onCalculatedTotalChange(total: string) {
        // this.calculatedTotal_Request = total; // รับเป็น string ที่ format แล้ว
        // let totalNumber = Number(total.replace(/,/g, ''));
        // if (totalNumber == undefined || totalNumber == null) {
        //     this.Budget_Request.Total = total;
        // }

        // if (typeof total === 'string' && total.includes(',')) {
        //     const totalNumber = Number(total.replace(/,/g, ''));
        //     this.Budget_Request.Total = totalNumber;
        //     this.calculatedTotal_Request = total;
        // } else {
        //     this.Budget_Request.Total = total;
        //     this.calculatedTotal_Request = total;
        // }
        this.calculatedTotal_Request = this.MasterService.show_fix(parseFloat(total)); // รับค่าจาก Rent_HomeComponent
    ...
    onUploadFileChange2(file: []): void { //รับ Upload File จาก Child component
        this.uploadedFiles2 = file; // เก็บไฟล์ที่อัพโหลด คุณลักษณะของครุภัณฑ์ (Spec)
    }
    ...
    uploadedFiles2: any = []; // เก็บไฟล์ที่อัพโหลด
    ...
    */
    ...
    choose_activity_dep_main() {
        ...
        if (Activity_Dep_Id != null && Activity_Dep_Id != undefined && Activity_Dep_Id !== "") {
    ...
    choose_activity_dep_second() {
        ...
        if (Activity_Dep_Second_Id != null && Activity_Dep_Second_Id != undefined && Activity_Dep_Second_Id !== "") {
    ...
    choose_activity_dep_sub() {
        ...
        if (Activity_Dep_Sub_Id != null && Activity_Dep_Sub_Id != undefined && Activity_Dep_Sub_Id !== "") {
    ...
    get_detail() {
        ...
        let model = {
            FUNC_CODE: 'FUNC-GET_DATA-BR_INVEST_FORM',
        ...
        getData.subscribe((response: any) => {
            if (response.RESULT == null) {
                ...
                this.List_Mas_Importance = response.List_Mas_Importance; // ความสำคัญ
                ...
                this.List_Mas_Month = response.List_Mas_Month; // เดือน
        ...
        if (this.Budget_Request.Fk_Plan_Id == null || this.Budget_Request.Fk_Plan_Id == undefined || this.Budget_Request.Fk_Plan_Id == 0) {
            ...
            getData.subscribe((response: any) => {
                ...
                if (response.RESULT == null) {
                    ...
                    //อัพโหลดไฟล์->แนบไฟล์ใบเสนอราคา
                    // 1 = ใบเสนอราคา ; 2 = คุณลักษณะของครุภัณฑ์ (Spec)
                    let Po_Type_File_Id = 0; //รายการสิ่งก่อสร้าง
                    if (this.Expense_Type_Id != 6) { //หมวดค่าใช้จ่าย->ครุภัณฑ์ มี upload 2 แบบ ->ไฟล์ใบเสนอราคา/คุณลักษณะของครุภัณฑ์ (Spec)
                        Po_Type_File_Id = 0 //ไฟล์ที่ไม่ใช้ Po
                    } else { //หมวดค่าใช้จ่าย->ครุภัณฑ์ มี upload 2 แบบ ->ไฟล์ใบเสนอราคา/คุณลักษณะของครุภัณฑ์ (Spec)
                        Po_Type_File_Id = 1 //แนบไฟล์ใบเสนอราคา
                    }
                    this.uploadedFiles1.forEach((file: any) => {
                        let uploadmodel = {
                            ...
                            Po_Type_File_Id: Po_Type_File_Id //แนบไฟล์ใบเสนอราคา
                        };
                    ...
                    //อัพโหลดไฟล์->คุณลักษณะของครุภัณฑ์ (Spec)
                    this.uploadedFiles2.forEach((file: any) => {
                        let uploadmodel = {
                            FUNC_CODE: "FUNC-UPLOAD_FILE",
                            Budget_Request_Detail_Item:
                                this.List_Budget_Request_Detail_Item[file.IndexFile],
                            Po_Type_File_Id: 2 //คุณลักษณะของครุภัณฑ์ (Spec)
                        };
                        const formData = new FormData();
                        formData.append("MODEL", JSON.stringify(uploadmodel));
                        formData.append("files", file);
