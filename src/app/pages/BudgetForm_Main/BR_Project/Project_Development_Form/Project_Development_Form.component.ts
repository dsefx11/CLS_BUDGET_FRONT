...
@Component({
    selector: 'Project_Development_Form',
    templateUrl: 'Project_Development_Form.component.html',
    providers: [GridJsService, DecimalPipe, EbudgetService],
    styleUrls: ['../BR_Project.component.scss'],
})

export class Project_Development_FormComponent implements OnInit {
    ...
    @Input() List_Mas_Country: any = []; //รับค่าจาก Component แม่
    @Input() List_Mas_Country_Group: any = []; //รับค่าจาก Component แม่
    @Input() Filter_List_Mas_Country: any = []; //รับค่าจาก Component แม่
    ...
    onChangeCheckBox_Agreement(currentValue: number, selectedValue: number): void {
        if (currentValue !== selectedValue) {
            this.Budget_Request_Detail.Agreement_Id = selectedValue; // ตั้งค่าตามที่เลือก
            if (selectedValue == 1) {
                this.Budget_Request_Detail.Agreement_Name = "มี";
            } else if (selectedValue == 2) {
                this.Budget_Request_Detail.Agreement_Name = "ไม่มี";
            }
        } else {
            this.Budget_Request_Detail.Agreement_Id = null; // ยกเลิกการเลือกหากคลิกซ้ำ
        }
    }
    ...
    choose_ddl_Mas_Country(data: any) {
        let c_item = this.List_Mas_Country.filter((item: any) => item.Country_Id == data.Country_Id)
        data.Country_Name = c_item[0].Country_Name;

        // กรองรายการที่เกี่ยวข้องกับระดับของแถวนี้และเก็บใน item_detial
        this.List_Mas_Country_Group = this.Filter_List_Mas_Country.filter((item: any) => item.Country_Id == data.Country_Id);
        data.Country_Group_Id = this.List_Mas_Country_Group[0].Country_Group; //SelectValue Add in Value Model
        data.Country_Group_Name = this.List_Mas_Country_Group[0].Country_Group_Name;

    }

    cal_qty_item(data: any) {
        let Total = data.Times * data.People * data.Qty_Day * data.Cost_Amount
        data.Total = Total;
