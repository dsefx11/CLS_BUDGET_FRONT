...
@Component({
    selector: 'SocialSecurity_Contribution_Form',
    templateUrl: 'SocialSecurity_Contribution_Form.component.html',
    providers: [GridJsService, DecimalPipe, EbudgetService],
    styleUrls: ['../BR_Operate.component.scss'],
})

export class SocialSecurity_Contribution_FormComponent implements OnInit {
    ...
    choose_ddl_department(data: any) {
        let c_item = this.List_Mas_Department.filter((item: any) => item.Department_Id == data.Department_Id)
    ...
    Cal_Salary_Amount(index: number) {
        let Total = (this.List_Budget_Request_Detail_Item[index].People * this.List_Budget_Request_Detail_Item[index].Salary_Amount) * 12;
