...
@Component({
    selector: 'Extra_Salary_Compensation_Form',
    templateUrl: 'Extra_Salary_Compensation_Form.component.html',
    providers: [GridJsService, DecimalPipe, EbudgetService],
    styleUrls: ['../BR_Operate.component.scss'],
})

export class Extra_Salary_Compensation_FormComponent implements OnInit {
    ...
    @Input() Budget_Request: any = {}; // รับค่าจาก Component แม่
    @Input() calculatedTotal_Front: string = ''; // รับค่าจาก Component แม่
    @Input() List_Mas_Position: any = []; // DDL ตำแหน่งงาน
    @Input() List_Mas_Position_Type: any = []; // DDL ประเภทตำแหน่งงาน
    ...
    constructor(private modalService: NgbModal, public service: GridJsService, private sortService: PaginationService, public MasterService: MasterService, public serviceebudget: EbudgetService, private cdr: ChangeDetectorRef) {
    ...
    choose_ddl_Position_Type(data: any) {
        let c_item = this.List_Mas_Position_Type.filter((item: any) => item.Position_Type_Id == data.Position_Type_Id)
        data.Position_Type_Name = c_item[0].Position_Type_Name;
    }

    cal_salary_year(index: number) {
        const total = this.List_Budget_Request_Detail_Item[index].Salary_Amount * 12
        this.List_Budget_Request_Detail_Item[index].Salary_Amount_Year = total;
        const withdraw = (this.List_Budget_Request_Detail_Item[index].Salary_Amount_Year * 4) / 100;
        this.List_Budget_Request_Detail_Item[index].Withdrawal_Amount = withdraw; //เบิกเพิ่ม
        const Sum_Total = this.List_Budget_Request_Detail_Item[index].Salary_Amount_Year + this.List_Budget_Request_Detail_Item[index].Withdrawal_Amount; //รวมเงินทั้งสิ้น
        this.List_Budget_Request_Detail_Item[index].Total = Sum_Total;
