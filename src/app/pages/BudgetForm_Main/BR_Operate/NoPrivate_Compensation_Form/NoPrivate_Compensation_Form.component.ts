...
@Component({
    selector: 'NoPrivate_Compensation_Form',
    templateUrl: 'NoPrivate_Compensation_Form.component.html',
    providers: [GridJsService, DecimalPipe, EbudgetService],
    styleUrls: ['../BR_Operate.component.scss'],
})

export class NoPrivate_Compensation_FormComponent implements OnInit {
    ...
    choose_ddl_Expense_Cost_Rate(index: number, data: any) {
        let c_item = this.List_Mas_Expense_Cost_Rate.filter((item: any) => item.Cost_Id == data.Fk_Cost_Id)
        data.Cost_Name = c_item[0].Cost_Name;

        this.List_Budget_Request_Detail_Item[index].Cost_Amount = c_item[0].Plan_Rate;

        let Total = this.List_Budget_Request_Detail_Item[index].Cost_Amount * 12
